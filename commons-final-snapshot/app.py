...
XENO_GAMY_ENGINE = os.path.join(MANTIC_SPACE, "projects", "xenogamy", "engine.py")
HYPOSTASIS_ENGINE = os.path.join(SEER_SPACE, "hypostasis", "engine.py")
...
<header>
  <nav>
    ...
    <a href="/hypostasis">Hypostasis</a>
...
class CommonsHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif path == "/hypostasis":
            self.handle_hypostasis()
        elif path == "/api/hypostasis":
            self.handle_hypostasis_api()
...
"""
    ...
    def handle_hypostasis(self):
        """The Hypostasis Engine — physicalize abstract concepts. Serves the static page."""
        self.send_response(302)
        self.send_header("Location", "/static/hypostasis.html")
        self.end_headers()

    def handle_hypostasis_api(self):
        """POST /api/hypostasis — run the Hypostasis Engine live."""
        import subprocess, os, json as j, tempfile
        ...
        concept = data.get("concept", "justice")
        
        try:
            tmpf = tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False)
            tmpf.close()
            
            cmd = ["python3", HYPOSTASIS_ENGINE,
                   "--concept", concept,
                   "--output", tmpf.name,
                   "--quiet"]
            ...
            proc = subprocess.run(
                ...
                cwd=os.path.dirname(HYPOSTASIS_ENGINE) or SEER_SPACE
            )
            ...
            if isinstance(result, list):
                result = result[0] if result else {"concept": concept, "error": "Empty result"}
            
            self.send_json(result)
            
        except subprocess.TimeoutExpired:
            self.send_json({"concept": concept, "error": "Timeout after 300s"}, code=500)
        except Exception as e:
            self.send_json({"concept": concept, "error": str(e)}, code=500)
...
""".format(
    ...
    def handle_xenogamy(self):
        """Redirect to the static Xenogamy page."""
        self.send_response(302)
        self.send_header("Location", "/static/xenogamy.html")
        self.end_headers()

    def handle_xenogamy_api(self):
        """POST /api/xenogamy — run the Xenogamy Engine live."""
        import subprocess, os, json as j, tempfile
        ...
        parent_a = data.get("parent_a", "Triangulation Engine")
        parent_b = data.get("parent_b", "Negation Engine")

        try:
            tmpf = tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False)
            tmpf.close()
            env = os.environ.copy()
            env["XENO_GAMY_RESULT"] = tmpf.name
            cmd = ["python3", XENO_GAMY_ENGINE, parent_a, parent_b]
            proc = subprocess.run(
                cmd,
                capture_output=True, text=True, timeout=300,
                cwd=os.path.dirname(XENO_GAMY_ENGINE),
                env=env,
            )
            try:
                with open(tmpf.name) as f:
            ...
            self.send_json({"error": "Engine timed out (300s)", "parent_a": parent_a, "parent_b": parent_b}, code=504)
        except Exception as e:
            self.send_json({"error": str(e), "parent_a": parent_a, "parent_b": parent_b}, code=500)
